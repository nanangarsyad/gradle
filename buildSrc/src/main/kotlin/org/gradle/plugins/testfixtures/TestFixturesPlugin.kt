...
open class TestFixturesPlugin : Plugin<Project> {
    ...
    fun Project.configureAsProducer() {
        ...
        configurations {
            ...
            "testFixturesUsageCompile" { extendsFrom(configurations["testFixturesCompile"], configurations["outputDirs"]) }
            "testFixturesUsageRuntime" { extendsFrom(configurations["testFixturesRuntime"], configurations["testFixturesUsageCompile"]) }
    ...
    fun Project.configureAsConsumer() = afterEvaluate {
        ...
        the<TestFixturesExtension>().origins.forEach { (projectPath, sourceSetName) ->
            ...
            val compileConfigurationName = if (sourceSetName == "main") "compile" else "${sourceSetName}Compile"
            val runtimeConfigurationName = if (sourceSetName == "main") "runtime" else "${sourceSetName}Runtime"

            dependencies {
                compileConfigurationName(project(path = projectPath, configuration = "testFixturesUsageCompile"))
                compileConfigurationName(project(":internalTesting"))
                runtimeConfigurationName(project(path = projectPath, configuration = "testFixturesUsageRuntime"))
            }
        }
    }
}
